...
export interface ProviderInputQueueSnapshot {
  readonly steering: ReadonlyArray<string>;
  readonly followUp: ReadonlyArray<string>;
}

export const EMPTY_PROVIDER_INPUT_QUEUE: ProviderInputQueueSnapshot = {
  steering: [],
  followUp: [],
};
...
export function deriveLatestProviderInputQueue(
  ...
  for (let index = (activities?.length ?? 0) - 1; index >= 0; index -= 1) {
    ...
    if (!payload) return EMPTY_PROVIDER_INPUT_QUEUE;
    return {
      steering: parseQueueMessages(payload.steering),
      followUp: parseQueueMessages(payload.followUp),
    };
  }
  return EMPTY_PROVIDER_INPUT_QUEUE;
}

export function providerInputQueueCount(snapshot: ProviderInputQueueSnapshot): number {
