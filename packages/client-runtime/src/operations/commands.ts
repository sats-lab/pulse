...
export type MutateThreadInputQueueInput = CommandInput<"thread.input-queue.mutate">;
...
export const mutateThreadInputQueue: (input: MutateThreadInputQueueInput) => CommandEffect =
  Effect.fn("EnvironmentCommands.mutateThreadInputQueue")(function* (input) {
    const metadata = yield* timestampedCommandMetadata(input);
    return yield* dispatch({
      ...input,
